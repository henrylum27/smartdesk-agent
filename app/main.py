...
def summarize_ticket(subject: str, body: str) -> dict:
    ...
    This is a lightweight, rule-based summarizer for the MVP.
    """
    text = f"{subject} {body}".lower()

    issue_keywords = {
        "Login / Access Issue": [
            "login", "log in", "access", "password", "account", "locked", "sign in"
        ],
        "Payment / Billing Issue": [
            "payment", "invoice", "billing", "refund", "charge", "bank", "subscription"
        ],
        "Technical Bug": [
            "error", "bug", "crash", "not working", "failed", "broken", "issue"
        ],
        "System Outage": [
            "down", "outage", "unavailable", "all users", "system is down"
        ],
        "Request / Account Change": [
            "change", "update", "request", "modify", "remove", "add"
        ],
        "Security Concern": [
            "security", "phishing", "unauthorized", "breach", "compromised", "suspicious"
        ]
    }
    ...
    if any(word in text for word in ["urgent", "immediately", "deadline", "all users", "unable to work"]):
        impact = "High business impact"

    if any(word in text for word in ["cannot access", "unable to access", "system is down", "payment system"]):
